...
$db->connect();
$id_func=$_GET[id];
$rs=$db->query("select id_per from ".TABLE2.",".TABLE22." where ".TABLE2.".id_func='$id_func' and ".TABLE22.".id_func='$id_func'");
$r=$db->fetch_array($rs);
$id_per=$r[id_per];

$rs3=$db->query("select id_sec from ".TABLE23." where id_per='$r[id_per]'");
...
$rs=$db->query("select * from ".TABLE2.",".TABLE22." where ".TABLE2.".id_func='$id_func' and ".TABLE22.".id_func='$id_func'");
...
if ($r=$db->fetch_array($rs))
{
	if (isset($_POST[save]))	
	{
		...
		$rs1=$db->query("insert into ".TABLE23." (id_per, name_sec, num_reg, date_afil) values ('$id_per','$name_medical_sec','$numb_secured','$date_start_sec')");
		if ($rs1)
		{
...
<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    <tr>
    <?			 	
		$rs2=$db->query("select charge from ".TABLE28." where id_per='$r[id_per]'");
		$r2=$db->fetch_array($rs2);
    ?>
      <th><div align="left">Cargo:</div></th>
      <td><div align="left"><strong><? echo $r2[charge]; ?></strong></div></td>
    </tr>
    <tr>
