...
class DB_MySQL {
  ...
  function try_connect($DBHost = "", $DBPort = "", $DBUser = "", $DBPassword = "") {
    ...
    if($this->Persistent)
      $this->Link_ID = @mysqli_connect("p:" . $DBHost, $DBUser, $DBPassword);
    else
  ...
  function connect($DBDatabase = "", $DBHost = "", $DBPort = "", $DBUser = "", $DBPassword = "") {
    ...
    if (!$this->Connected) {
      ...
      if($this->Persistent)
        ...
        $this->Link_ID=@mysqli_connect($DBHost, $DBUser, $DBPassword, $DBDatabase);
      ...
      if ($version[0] >= 4) {
        if (($version[0] > 4 || $version[1] >= 1) && is_array($this->Encoding) && $this->Encoding[1])
          @mysqli_query($this->Link_ID, "SET NAMES '" . $this->Encoding[1] . "'");
        elseif (is_array($this->Encoding) && $this->Encoding[0])
          @mysqli_query($this->Link_ID, "SET NAMES '" . $this->Encoding[0] . "'");
      }
      $this->Connected = true;
    }
